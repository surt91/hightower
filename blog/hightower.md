...
Das Paper ist auch als Zeitdokument lesenswert. Hightower hat sein Programm auf Platinen losgelassen, auf PERT-Diagramme und auf Labyrinthe, darunter eine rechtwinklige Fassung des Heckenlabyrinths von Hampton Court. Die Plots im Anhang stammen von einem *Dataplotter* der Electronics Associated Inc., und unter dem Hampton-Court-Bild steht „Total time .0005" (die Einheit steht nicht dabei, vermutlich Stunden, was zu den Abrechnungsgewohnheiten der Zeit passen würde). Dieses Labyrinth wollte ich exakt nachbauen. Der Scan des Papers liegt mit 300 dpi vor, die dünnen Linien sind die Wände, die dicke Linie ist Hightowers gefundener Pfad. Ein kurzes Python-Skript trennt die beiden nach Strichstärke, ordnet jedes Tintenpixel einem waagerechten oder senkrechten Strich zu, schließt die Lücken an den Kreuzungen und skaliert das Ergebnis auf ganze Zahlen (vier Pixel sind eine Einheit). Heraus kommen 54 Wandstücke in einem Feld von 224 × 212 Einheiten (sieben Scanpixel pro Einheit, die Gänge sind etwa drei Einheiten breit), dazu die Lage von A und B, abgelesen an den freien Enden der dicken Linie.

![Das aus dem Scan nachgezeichnete Labyrinth: dünne schwarze Wände, Hightowers Pfad von 1969 hellgrau unterlegt, der Pfad dieser Implementierung in Rot, darüber in Schreibmaschinenschrift die Beschriftung des Originals mit der neuen Zeit.](images/03_maze.svg)

*Das aus dem Scan nachgezeichnete Labyrinth. Grau Hightowers Pfad von 1969, rot der Pfad dieser Implementierung. Die Zeit steht in seiner Einheit: rund 0,0000005 Stunden, also knapp zwei Millisekunden, für 151 Linien in 39 Schritten.*

Dass die rote Linie nicht überall der grauen folgt, liegt nicht an der Rekonstruktion. Der Algorithmus hat an einigen Stellen Wahlfreiheit, etwa welches von zwei Hindernisenden er zuerst probiert, und wo sich die beiden Netze treffen, entscheidet über den Verlauf. Dass er das Labyrinth überhaupt löst, war weniger selbstverständlich, als ich dachte. Dazu weiter unten mehr.
...
**Prozess II** greift, wenn kein Cover-Ende erreichbar ist, etwa weil Z in einer Tasche sitzt. Dann wandert der Algorithmus von den Enden der eigenen Escape-Linien aus Einheit für Einheit zurück in Richtung Z. An jeder Position zieht er eine senkrechte Sondenlinie und versucht von dort aus Prozess I. Man kann sich das als Rückzug entlang der Wand vorstellen, bei dem man immer wieder über die Schulter schaut, ob sich eine Lücke auftut. In meiner Fassung geht der Rückzug außerdem rekursiv weiter: Führt weder die Sondenlinie noch Prozess I an ihrer Position weiter, wandert der Algorithmus auch entlang der Sondenlinie zurück und probiert dort erneut. Das Paper formuliert diesen Punkt knapp („try to find a Process I escape point on either of the two escape lines as outlined in the Escape Algorithm"), und man kann ihn so oder so lesen. Ohne die Rekursion bleibt der Router im Hampton-Court-Labyrinth in der ersten langen Sackgasse stecken, mit ihr findet er den Weg. Da Hightower das Labyrinth 1969 gelöst hat, halte ich die rekursive Lesart für die richtige.
...
Der Pfad, den man aus den Bäumen abliest, ist gültig, aber nicht immer schön. Escape-Linien sind maximal lang, und ein Kreuzungspunkt liegt gern weiter außen als nötig, sodass der Pfad einen Umweg macht und wieder zurückkommt. In der Beispielszene ist es der kleine Haken rechts von B: Die Linie von A trifft die horizontale Linie von B ein Stück rechts von B, und der Pfad läuft an B vorbei, biegt ab und kommt zurück. Hightower nennt die Reparatur *second improvement* und beschreibt zwei Varianten. Bei der ersten verlängert man jedes Pfadsegment so weit, wie die Hindernisse es zulassen, und wenn die Verlängerung ein späteres, senkrechtes Pfadsegment trifft, schneidet man alles dazwischen heraus. Bei der zweiten schiebt man einen Tastpunkt Einheit für Einheit über jedes Segment und schießt von dort senkrechte Linien ab. Trifft eine davon ein späteres paralleles Segment, wird auch hier abgekürzt. Die zweite Variante ist teurer, weil sie mit der Pfadlänge skaliert, aber sie entfernt Haken wie den bei B, die der ersten entgehen. Sie repariert außerdem eine Eigenart des rekursiven Rückzugs: Der startet immer am fernen Ende einer Linie, sodass der Rohpfad in einem Gang von Wand zu Wand pendelt. Im Labyrinth ist der Rohpfad 5095 Einheiten lang, nach der ersten Variante 1933, nach der zweiten 1357, und das entspricht hier der Länge des kürzesten Pfads. Beide Varianten laufen deshalb standardmäßig, und das Tasten beschränkt sich auf Positionen, von denen aus überhaupt ein späteres paralleles Segment getroffen werden kann.
...
Der Grund für dieses Beispiel ist ein Detail in Prozess II. Hightower definiert die Rückzugspositionen als Schnittpunkte der Escape-Linie mit ihren Covern. Endet die Linie am Spielfeldrand, gibt es kein Cover und damit keinen Rückzug. Eine Variante, die sich auch von Rändern zurückzieht, findet diese Szene und drückt die Fehlerquote auf 0,1 Prozent, zeichnet dafür in offenen Szenen aber Hunderte Linien, weil sie an einer Linie von Rand zu Rand jede einzelne Position abklappert. Das ist wieder ein Gitter, nur durch die Hintertür. Im Crate ist sie als Option erhalten, die Voreinstellung folgt dem Paper.
...
![Zwei Liniendiagramme mit logarithmischer Zeitachse. Links wächst die Kantenlänge des Spielfelds von 64 auf 2048 bei 20 Hindernissen: die Breitensuche steigt von rund 80 Mikrosekunden auf 58 Millisekunden, der Sichtbarkeitsgraph bleibt bei 16 bis 23 Mikrosekunden, Hightower bei ein bis zwei Mikrosekunden. Rechts wächst die Zahl der Rechtecke von 0 auf 320 bei fester Fläche: die Breitensuche bleibt bei einer halben bis einer Millisekunde, der Sichtbarkeitsgraph steigt von unter einer Mikrosekunde auf über eine halbe Millisekunde, Hightower von 0,4 auf etwa 28 Mikrosekunden.](images/12_benchmark.svg)

*Links: Die Fläche wächst, die Anzahl der Hindernisse bleibt. Rechts: Die Fläche bleibt, die Hindernisse werden mehr.*

Die linke Kurve ist der Grund für diesen Artikel. Bei 20 Rechtecken braucht Hightower ein bis zwei Mikrosekunden pro Route, und es ist ihm egal, ob das Spielfeld 64 oder 2048 Einheiten breit ist. Dem Sichtbarkeitsgraphen ist es ebenso egal, er liegt konstant bei etwa 16 bis 23 Mikrosekunden, also rund zehnmal über Hightower. Die Breitensuche skaliert mit der Fläche und landet bei 2048 Einheiten Kantenlänge bei 58 Millisekunden, was für einen interaktiven Editor mit vielen Kanten zu langsam wäre.

Rechts sieht man, was die beiden gitterfreien Verfahren kostet. Mit der Anzahl der Hindernisse wachsen die Kosten bei Hightower, weil jede Cover-Abfrage durch mehr Zeilen läuft und Prozess II mehr Linien probiert; beim Sichtbarkeitsgraphen wächst die Knotenzahl quadratisch, und A* muss entsprechend mehr davon anfassen. Bei 160 Rechtecken auf 256 × 256 Einheiten ist die Szene so voll, dass der Sichtbarkeitsgraph länger braucht als die Breitensuche, während Hightower bei 28 Mikrosekunden liegt. Ein Diagramm-Editor hätte in dieser Szene allerdings andere Sorgen. In der Spalte ganz rechts fällt außerdem auf, dass bei 160 Rechtecken 16 der Zufallsszenen von Hightower nicht gelöst wurden; die Messung zählt nur Szenen, die alle drei Router schaffen.
